...
def gen_bins(fastafile,resultfile,outputdir):
    ...
    for _,cluster in dic.items():
        if bin_name < 10:
            bin = 'REFINE'+ '000' + str(bin_name) + '.fa'
        elif bin_name >= 10 and bin_name < 100:
            bin = 'REFINE'+ '00' + str(bin_name) + '.fa'
        elif bin_name >= 100 and bin_name < 1000:
            bin = 'REFINE'+ '0' + str(bin_name) + '.fa'
        else:
            bin = 'REFINE'+str(bin_name) + '.fa'
...
def bin_evaluation(gp , bin_type ,ref_tax , ref_len):
    true = []
    pred = []
    for i in range(len(gp)):
        for j in gp[i]:
            if j in ref_tax:
                pred.append(i)
                true.append(ref_tax[j])
    
    FW_score = metrics.fowlkes_mallows_score(true, pred)
    ARI_score = metrics.adjusted_rand_score(true, pred)
    NMI_score = metrics.normalized_mutual_info_score(true, pred)
    
    logger.info('\n')
    logger.info('FW score for {} is {}'.format(bin_type , FW_score))
    logger.info('ARI score for {} is {}'.format(bin_type , ARI_score))
    logger.info('NMI score for {} is {}'.format(bin_type , NMI_score))
    ...
    logger.info('length for {} is {}'.format(bin_type , length))
    logger.info('count for {} is {}'.format(bin_type , count))
    logger.info('fraction for {} is {}'.format(bin_type , length/sum(ref_len.values())))
