...
if __name__ == '__main__':
    ...
    if args.command == 'refine':
        ...
        gp_merge = merge(gp_share_plus , ref_ind , map_matrix , ifelse(args.alpha, runtime_defaults['alpha']))
        bin_evaluation(gp_merge , 'merged_bin' , ref_tax , ref_len)

        ##########Step3 assign short contigs##############
        logger.info('Begin Step 3...')
        gp_final = assign(gp_merge , map_matrix , ref_ind , 0.1)
        bin_evaluation(gp_final , 'final_bin_0.1' , ref_tax , ref_len)
        
        gp_final = assign(gp_merge , map_matrix , ref_ind , 0.2)
        bin_evaluation(gp_final , 'final_bin_0.2' , ref_tax , ref_len)
        
        gp_final = assign(gp_merge , map_matrix , ref_ind , 0.3)
        bin_evaluation(gp_final , 'final_bin_0.3' , ref_tax , ref_len)
        
        gp_final = assign(gp_merge , map_matrix , ref_ind , 0.4)
        bin_evaluation(gp_final , 'final_bin_0.4' , ref_tax , ref_len)
        
        gp_final = assign(gp_merge , map_matrix , ref_ind , 0.5)
